 ...
 */
async function handleEscapeKey(e) {
  ...
  if (expandedSection && isDesktop.matches) {
    ...
    await toggleMenu(nav, navSections, false);
 ...
 */
async function handleFocusLost(e) {
...
async function toggleMenu(nav, navSections, forceExpanded = null) {
  ...
  document.body.style.overflowY =
    !shouldExpand || isDesktop.matches ? "" : "hidden";
...
function setupSubmenuHoverEvents(navSection) {
  ...
  // Only target direct children of the main dropdown (submenu items like "General Insurance", "Life Insurance")
  const submenuItems = mainDropdown.querySelectorAll(":scope > li");
  
  submenuItems.forEach((submenuItem) => {
    // Only target the immediate children ul (like "Motor Insurance", "Non Motor Insurance" containers)
...
export default async function decorate(block) {
  console.log("Decorating header navigation");
